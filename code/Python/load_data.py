...
def get_int_box_data(field):
    ...
    if f != None:  
        ...
        cfg.pms['zMpc'] = int(box_sizez); cfg.pms['xyMpc'] = int(box_sizexy)
    ...
    np.save('{5}{0}zstart{1}_zend{2}_FLIPBOXES1_{3}_{4}Mpc_lighttravel_cat'.format(cfg.box_headers[field],cfg.pms['zi'],cfg.pms['zf'],int(box_zMpc),int(box_size),cfg.data_dir),catbox)
    return catbox 
